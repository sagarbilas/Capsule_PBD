...
function getCapsuleBodyNormal(agent, agentLength, RADIUS, current_rotation) {
  ...
  function comfortConstraint_Capsule(best_i, best_j, p_best_i, p_best_j, capsule_i, capsule_j) 
  {
    ...
    else if(customParams.scenario == 'swap_Scenario')
    {
      ...
      stif = 0.019;       //  0.011;    0.021;    0.020   0.024;
    ...
    {
      // if (agentDist < 3.6 ) 
      if (agentDist < 2.5 )       // 2.4    2.15   2.06   2.58
      {  
...
{ 
  ...
  }else if( ((clearance) < (2 * cross_width )) && ( clearance - capsule.radius > 0) && (activate_orientation == 'true') )
  {
    ...
    // if( (Math.abs(capsule.agent.rotation.z - angleInRadians) >= 0.08)  && (angleBodyNormalToGoalVec2 < 30) )
  ...
  // else if(  (clearance > 50) )
  {
    ...
    if(customParams.scenario == 'suddenStop')
    {
      ...
      */
      ...
      console.log(" capsule.agent.rotation.z: ", capsule.agent.rotation.z);

      // if( capsule.agent.rotation.z < 4.0)
      // if( capsule.agent.rotation.z < 3.75)
      if( capsule.agent.rotation.z < 3.6)
      {
        let next_orientation = capsule.agent.rotation.z - 1.57 * 2;
        capsule.agent.rotation.z = capsule.agent.rotation.z + 1.57 * 0.05;
        console.log("here.....")
        //----------------------------------------------------------------------------------------
        ...
        // {
        //   capsule.agent.rotation.z = cur_orientation + (cur_orientation - next_orientation)/50 ;    //250  1200
        // }else{
        //   capsule.agent.rotation.z = cur_orientation -  (cur_orientation - next_orientation)/50 ;    //250   1200
        // }
    }
