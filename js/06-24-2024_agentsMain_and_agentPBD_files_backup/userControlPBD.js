...
function makeNextAgentActive( agent_index ) 
{
  ...
  function rotationConstraint(capsule_entity)
  {
    ...
    {  
      console.log("rotation constraint activated!!");
  ...
  function findClosestLeftOrRightPointOfVel(capsule_entity, point_on_obs)
  {
    ...
    //--------------------------------------------------------------------
    // let velocity_vec = new THREE.Vector3(capsule_entity.vx, 0, capsule_entity.vz);
    // // Compute the dot product between the velocity and vectorToP
    // // let dotProduct_vel_pos = velocity_vec.dot(vectorToPoint);
    // let dotProduct_vel_pos = vel_direction.dot(vectorToPoint);
    // // console.log("dotProduct_vel_pos: ", dotProduct_vel_pos);
    //--------------------------------------------------------------------

    if(direction == 'right')
    // if( direction == 'right' && dotProduct_vel_pos > 0 )
    {
      // console.log("id: ", capsule_entity.index, "hi1");
      ...
      // console.log("id: ", capsule_entity.index, ", capsule_entity.closest_agent_in_right: ", capsule_entity.closest_agent_in_right, ", direction: ", direction);
        
      return [capsule_entity.closest_agent_in_right, direction]
    }
    
    if(direction == 'left' )  
    // if(direction == 'left' && dotProduct_vel_pos > 0 )             
    {
      // console.log("id: ", capsule_entity.index, "hi2");
  ...
  function findClosestLeftOrRightWALLPointOfVel_v2(capsule_entity)
  {
    ...
    // console.log("closest_wall_in_left: ", capsule_entity.closest_wall_in_left, ", closest_wall_in_right: ", capsule_entity.closest_wall_in_right);
...
return [closest_in_left,  closest_in_right, distToActivateOrientationConstraint, activate_orientation]            
  ...
  /*
  let cur_position = new THREE.Vector3(capsule_entity.x, 0, capsule_entity.z);
  // Calculate the coordinates of point B in the opposite direction
  var pointBehindCurPos = new THREE.Vector3();
  let distanceAtBehind = 1;

  let velocity_vec = new THREE.Vector3(capsule_entity.vx, 0, capsule_entity.vz);
  let vel_direction = new THREE.Vector3(capsule_entity.vx, 0, capsule_entity.vz).normalize();

  pointBehindCurPos.addVectors(cur_position, vel_direction.multiplyScalar(-distanceAtBehind));

  // Create vector from capsule position to the point
  // let vectorToclosest_in_right_2 = new THREE.Vector3().subVectors(closest_in_right_2, cur_position); 
  let vectorToclosest_in_right_2 = new THREE.Vector3().subVectors(pointBehindCurPos, closest_in_right_2);  
  // let dotProduct_vel_pos_right = velocity_vec.dot(vectorToclosest_in_right_2);
  let dotProduct_vel_pos_right = vel_direction.dot(vectorToclosest_in_right_2);

  // var dotProduct_vel_pos_right = vectorToclosest_in_right_2.dot(vel_direction);

  if(dotProduct_vel_pos_right < 0 )
  {
    console.log("id: ", capsule_entity.index, ", dotProduct_vel_pos_right: ", dotProduct_vel_pos_right);
    console.log("pointBehindCurPos: ", pointBehindCurPos, ", cur_position: ", cur_position , ", closest_in_right_2: ", closest_in_right_2 );
    console.log("vel_direction: " , vel_direction, "\n\n ");
    console.log(" \n\n ");
  }

  // const vectorToclosest_in_left_2 = new THREE.Vector3().subVectors(closest_in_left_2, cur_position);
  let vectorToclosest_in_left_2 = new THREE.Vector3().subVectors( pointBehindCurPos, closest_in_left_2);
  let dotProduct_vel_pos_left = vel_direction.dot(vectorToclosest_in_left_2);
  // var dotProduct_vel_pos_left = vectorToclosest_in_left_2.dot(vel_direction);

  if(dotProduct_vel_pos_left < 0 )
  {
    console.log("id: ", capsule_entity.index, ", dotProduct_vel_pos_left: ", dotProduct_vel_pos_left );
    console.log("pointBehindCurPos: ", pointBehindCurPos, ", cur_position: ", cur_position , ", closest_in_left_2: ", closest_in_left_2 );
    console.log("vel_direction: " , vel_direction, "\n\n ");
    console.log(" \n\n ");
  }
  */
  ...
  let vel_direction_unit = new THREE.Vector3(capsule_entity.vx, 0, capsule_entity.vz).normalize();

  // Calculate the coordinates of point B in the opposite direction
  let pointBehindCurPos = new THREE.Vector3();
  let distanceAtBehind = 3;
  pointBehindCurPos.addVectors(cur_position, vel_direction_unit.multiplyScalar(-distanceAtBehind));

  let pointBehindPredictedPos = new THREE.Vector3();
  pointBehindPredictedPos.addVectors(predicted_position, vel_direction_unit.multiplyScalar(-distanceAtBehind));

  // Calculate the vectors from the current and future capsule positions to point P
  let vectorCurrent_right = new THREE.Vector3();
  vectorCurrent_right.subVectors(closest_in_right_2, pointBehindCurPos);

  let vectorCurrent_left = new THREE.Vector3();
  vectorCurrent_left.subVectors(closest_in_left_2, pointBehindCurPos);

  let vectorFuture_right = new THREE.Vector3();
  // vectorFuture_right.subVectors(closest_in_right_2, predicted_position);
  vectorFuture_right.subVectors(closest_in_right_2, pointBehindPredictedPos);
  
  let vectorFuture_left = new THREE.Vector3();
  // vectorFuture_left.subVectors(closest_in_left_2, predicted_position);
  vectorFuture_left.subVectors(closest_in_left_2, pointBehindPredictedPos);

  // Calculate the lengths of these vectors
  let lengthCurrent_right = vectorCurrent_right.length();
  let lengthFuture_right = vectorFuture_right.length();

  let lengthCurrent_left = vectorCurrent_left.length();
  let lengthFuture_left = vectorFuture_left.length();

  let distToRight = distance(pointBehindCurPos.x, pointBehindCurPos.z, closest_in_right_2.x, closest_in_right_2.z);
  let distToLeft = distance(pointBehindCurPos.x, pointBehindCurPos.z, closest_in_left_2.x, closest_in_left_2.z);
  
  if(lengthCurrent_right < lengthFuture_right )
  {
    // console.log("id: ", capsule_entity.index, ", distToRight: ", distToRight);
    // console.log("cur_position: ", cur_position, ", pointBehindCurPos: ", pointBehindCurPos, ", predicted_position: ", predicted_position , ", closest_in_right_2: ", closest_in_right_2 );
    // console.log(" \n\n ");
  }
  
  if(lengthCurrent_left < lengthFuture_left )
  {
    // console.log("id: ", capsule_entity.index, ", distToLeft: ", distToLeft);
    // console.log("cur_position: ", cur_position, ", pointBehindCurPos: ", pointBehindCurPos, ", predicted_position: ", predicted_position , ", closest_in_left_2: ", closest_in_left_2 );
    // console.log(" \n\n ");
  }

  if(capsule_entity.index == 1)
  {
    // console.log("id: ", capsule_entity.index, ", cur_position: ", cur_position, ", closest_in_right_2: ", closest_in_right_2, ", closest_in_left_2: ", closest_in_left_2 );
  }
  //----------------  WAY 2 ------ END -----------------------------------------------------------------------------------------
...
*/
  ...
  if(capsule_entity.index == 0)
    {
      // console.log("id: ", capsule_entity.index, ", cur_position: ", cur_position, ", closest_in_right_2: ", closest_in_right_2, ", closest_in_left_2: ", closest_in_left_2 );
      // console.log("distCurToRight: ", distCurToRight, ", distCurToLeft: ", distCurToLeft );
      // console.log(" \n\n ");
    }
  //----------------  WAY 3 ------ END -----------------------------------------------------------------------------------------
  ...
  // if( distCurToRight > 6 )
  // {
  //   console.log("id: ", capsule_entity.index, ", cur_position: ", cur_position, ", closest_in_right_2: ", closest_in_right_2 );
  //   console.log("distCurToRight: ", distCurToRight );
  //   console.log(" \n\n ");
  // }
  // if( distCurToLeft > 6 )
  // {
  //   console.log("id: ", capsule_entity.index, ", cur_position: ", cur_position, ", closest_in_left_2: ", closest_in_left_2 );
  //   console.log( "distCurToLeft: ", distCurToLeft );
  //   console.log(" \n\n ");
  // }
    ...
    // if(closest_in_right_2.length() != 0 && dotProduct_vel_pos_right > 0 )
    // if(closest_in_right_2.length() != 0 && lengthCurrent_right >= lengthFuture_right)
    if(closest_in_right_2.length() != 0 && distCurToRight < 6)
    {
      ...
      if(capsule_entity.index == 0)
      {
        // console.log("id: ", capsule_entity.index, ", closest_in_right_2: ", closest_in_right_2 );
    ...
    // if(closest_in_left_2.length() != 0 && dotProduct_vel_pos_left > 0 )
    // if(closest_in_left_2.length() != 0 && lengthCurrent_left > lengthFuture_left )
    if(closest_in_left_2.length() != 0 && distCurToLeft < 6)
    {
      perpendicularDistance_left = compute_Shortest_Perpendicular_dist(closest_in_left_2, current_position, normalized_velocity_2);

      if(capsule_entity.index == 0)
      {
        // console.log("id: ", capsule_entity.index, ", closest_in_left_2: ", closest_in_left_2 );
        // console.log("cur_position: ", cur_position, "\n\n" );
      }
...
{ 
  ...
  if(capsule.index == 0)
  {
    // console.log("id: ", capsule.index, ", clearance: ", clearance);
  }
  ...
  }else if( ((clearance) < (2 * cross_width )) && ( clearance - capsule.radius > 0) && (activate_orientation == 'true') )
  {
    ...
    if(capsule.index == 0)
    {
      console.log("id: ", "angleInRadians: ", angleInRadians );
    }
    ...
    // if( (Math.abs(capsule.agent.rotation.z - angleInRadians) >= 0.08)  && (angleBodyNormalToGoalVec > -70 && angleBodyNormalToGoalVec < 70) )
    // if( (Math.abs(capsule.agent.rotation.z - angleInRadians) >= 0.08)  && (angleBodyNormalToGoalVec > 1 && angleBodyNormalToGoalVec < 120) )

    // if( (Math.abs(capsule.agent.rotation.z - angleInRadians) >= 0.08)  && (angleBodyNormalToGoalVec2 > 1 && angleBodyNormalToGoalVec2 < 90) )
    if( (Math.abs(capsule.agent.rotation.z - angleInRadians) >= 0.08)  && (angleBodyNormalToGoalVec2 < 90) )
    //------------------------------------------------------------------------------------------------------
    //smooth the rotation while changing orientation
  ...
  // else if(  (clearance > 50) )
  {
    ...
    if(customParams.scenario == 'suddenStop')
    {
      ...
      if( capsule.agent.rotation.z < 3.1)
      {
        capsule.agent.rotation.z = capsule.agent.rotation.z + 1.57/500;    // 1.57/250;
        // capsule.agent.rotation.z = capsule.agent.rotation.z + 1.57 ;
        // capsule.agent.rotation.z = capsule.agent.rotation.z / 20 ;
      } 
    }else{
    if( capsule.agent.rotation.z < 3.1)
    {
    ...
    customParams.orientation = 'side_step';
      ...
      // console.log("else, , id: ", capsule.index, ", capsule.agent.rotation.z: ", capsule.agent.rotation.z, ", activate_orientation: ", activate_orientation );
...
/*    
    ...
    if(item.index == 1)
    {
      console.log("id:: ", item.index, ", cur_orientation: ", cur_orientation, ", next_orientation: ", next_orientation);
    }
    

    Math.abs(cur_orientation - next_orientation) >= 0.08
    {
      if( customParams.orientation == 'front')
        {
...
if( customParams.orientation == 'front' && angleBodyNormalToGoalVec > 1 )
{
  ...
  if(item.index == 0)
  {
    // console.log("id: ", capsule.index, ", angleBodyNormalToGoalVec2: ", angleBodyNormalToGoalVec2 );
    console.log("id: ", item.index, ", item.agent.rotation.z: ", item.agent.rotation.z );    
  }
